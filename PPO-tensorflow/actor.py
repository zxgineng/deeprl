...
def _random_normal_initializer(num):
    return tf.random_normal_initializer(stddev=np.sqrt(1 / num))
...
class Actor:
    ...
    def _build_graph(self):
        ...
        if Config.data.action_type == 'discrete':
            ...
            net = slim.fully_connected(net, hidden2, tf.nn.tanh,
                                       weights_initializer=_random_normal_initializer(hidden1))
            net = slim.fully_connected(net, hidden3, tf.nn.tanh,
                                       weights_initializer=_random_normal_initializer(hidden2))
            logits = slim.fully_connected(net, Config.data.action_num, activation_fn=None,
                                          weights_initializer=_random_normal_initializer(hidden3))
